...
public class CharacterFacadeREST extends AbstractFacade<Character> {
    ...
    @Path("{id}")
    @Consumes({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public void edit(@PathParam("id") Integer id, Character entity) {
        super.edit(entity);
    }
    ...
    public Character find(@PathParam("id") Integer id) {
    ...
    @Path("{from}/{to}")
    @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public List<Character> findRange(@PathParam("from") Integer from, @PathParam("to") Integer to) {
