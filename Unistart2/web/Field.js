...
// Model for Field entity
    models.Field = Backbone.Model.extend({
        urlRoot: "http://localhost:8084/Unistart2/webresources/model.field/",
        idAttribute: 'fieldId',
        defaults: {
            fieldName: "",
            fieldCode: ""
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('fieldId');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8084/Unistart2/webresources/model.field/';
            }
    ...
    // Collection class for Field entities
    models.FieldCollection = Backbone.Collection.extend({
        model: models.Field,
        url: "http://localhost:8084/Unistart2/webresources/model.field/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.FieldCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-field-list-itemis template identifier for item
                        templateName: '#tpl-field-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.field = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.field,
                    // tpl-field-details is template identifier for chosen model element
                    templateName: '#tpl-field-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Field();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-field-details is a template identifier for chosen model element
                templateName: '#tpl-field-details',
