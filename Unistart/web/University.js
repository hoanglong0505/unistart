...
// Model for University entity
    models.University = Backbone.Model.extend({
        urlRoot: "http://localhost:8084/Unistart/webresources/model.university/",
        idAttribute: 'universityId',
        defaults: {
            universityName: "",
            website: "",
            levelId: "",
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('universityId');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8084/Unistart/webresources/model.university/';
            }
    ...
    // Collection class for University entities
    models.UniversityCollection = Backbone.Collection.extend({
        model: models.University,
        url: "http://localhost:8084/Unistart/webresources/model.university/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.UniversityCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-university-list-itemis template identifier for item
                        templateName: '#tpl-university-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.university = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    ...
                    // tpl-university-details is template identifier for chosen model element
                    templateName: '#tpl-university-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.University();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-university-details is a template identifier for chosen model element
                templateName: '#tpl-university-details',
