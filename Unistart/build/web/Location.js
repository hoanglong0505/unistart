...
// Model for Location entity
    models.Location = Backbone.Model.extend({
        urlRoot: "http://localhost:8084/Unistart/webresources/model.location/",
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('locationId');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8084/Unistart/webresources/model.location/';
            }
    ...
    // Collection class for Location entities
    models.LocationCollection = Backbone.Collection.extend({
        model: models.Location,
        url: "http://localhost:8084/Unistart/webresources/model.location/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.LocationCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-location-list-itemis template identifier for item
                        templateName: '#tpl-location-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.location = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.location,
                    // tpl-location-details is template identifier for chosen model element
                    templateName: '#tpl-location-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Location();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-location-details is a template identifier for chosen model element
                templateName: '#tpl-location-details',
